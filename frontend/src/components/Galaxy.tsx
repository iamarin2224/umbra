...
float Star(vec2 uv, float flare) {
  float d = length(uv);
  float m = (0.035 * uGlowIntensity) / (d + 0.002);
  float rays = smoothstep(0.0, 1.0, 1.0 - abs(uv.x * uv.y * 1400.0));
  m += rays * flare * uGlowIntensity * 0.45;
  uv *= MAT45;
  rays = smoothstep(0.0, 1.0, 1.0 - abs(uv.x * uv.y * 1400.0));
  m += rays * 0.25 * flare * uGlowIntensity * 0.45;
  m *= smoothstep(0.55, 0.025, d);
  return m;
}
...
void main() {
  ...
  for (float i = 0.0; i < 1.0; i += 1.0 / NUM_LAYER) {
    ...
    float scale = mix(24.0 * uDensity, 0.9 * uDensity, depth);
  ...
  if (uLightMode > 0.5) {
    ...
    float alpha = clamp(length(col) * 2.2, 0.0, 1.0);
